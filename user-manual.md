...
#### Fixing a Wrong Cost (Long-Press the Cost Line)

Tony fat-fingered a cost during testing? The customer negotiated a different price? You can correct any costed item's price right from the item's repair history — no admin unlock required, and it works the same way on Front Counter and on Tech Station.

1. Open the item card on the job detail view (or the item detail view on Tech Station)
2. Scroll down to the **Repair History** timeline — at the bottom you'll see the cost line, e.g. **💲 $350.00** (or a Parts / Labor / Tax / Total breakdown if split is on)
3. **Long-press** (press and hold for about half a second) on that cost line
4. Tap **Edit Cost** in the menu that appears — the **Edit Cost** sheet opens pre-filled with the current values
5. Adjust the total (or parts/labor breakdown if split is on), toggle taxable if needed, tap **Save**
6. A confirmation popup appears: **"Was that cost approved by a manager?"** — same as the normal cost entry flow
   - **Yes** → the cost updates and the change is logged
   - **No — I'll confirm first** → nothing changes; come back when you've got the OK

The cost updates immediately and the cost summary card up top recalculates. The item's status doesn't change — if it was Complete it stays Complete.

**Every approved cost change is logged.** A timestamped note is automatically added to the item's notes timeline showing the old amount, the new amount, and the iPad + role. For example:

> **System** — Cost updated from $350.00 to $325.00 by Front Counter iPad (front_counter)

This is the universal "we caught a mistake" path — anyone on any device can use it on items at Complete or Closed. The orange **Edit Cost** button on the item card (admin-only, requires admin unlock) is still there for in-flow corrections before close, but for the common shop-floor "wrong number, fix it now" case, the long-press is the right tool.
...
**Caught a mistake after submitting?** Once an item is at Complete or Closed, you (or anyone on any device) can correct the cost via long-press on the **💲 $XX.XX** line in the item's Repair History timeline → **Edit Cost** → adjust → **Save** → confirm the manager-approval prompt. See **Fixing a Wrong Cost (Long-Press the Cost Line)** in the Front Counter section for the full walkthrough — same gesture, same sheet, available right here on Tech Station.
...
### Rolling Back an Item's Status (Admin Only)

Sometimes an item gets advanced one step too far — a tech taps **Save & Start Repair** during disassembly when they meant Save, or someone hits **Tested** before the test was actually finished. Rather than calling the office to fix it in the database, an admin can roll the item back one step right from the detail view.

**How to roll back:**

1. Make sure admin access is active (admin-role device, or elevate with your PIN)
2. Open the item's detail view (from the Job Board on Front Counter, or by tapping the item from Tech Station)
3. **Long-press** on the **Repair History** label in the timeline section
4. A menu appears with one option: **Undo — Back to [previous status]** — for example, "Undo — Back to Disassembly" if the item is currently In Progress
5. Tap it → a confirmation alert spells out exactly what's about to happen, e.g. *"#1234.A will be moved from In Progress back to Disassembly. This cannot be undone."*
6. Tap **Roll Back** to confirm, or **Cancel** to back out

**What rollback does at each step:**

Rollback only moves the item one step at a time along the linear repair flow:

- **Disassembly → Checked In** — clears the disassembly state
- **In Progress → Disassembly** — clears the repair-done flag and the repair end timestamp
- **Tested → In Progress** — clears the tester, the tested timestamp, and the repair-done flag
- **Complete → Tested** — clears all cost fields (total, parts/labor split, taxable flag, tax amount, manager approval). The cost will need to be re-submitted before the item can move to Complete again
- **Closed → Complete** — clears the closure timestamp and who closed it. This is the same transition the existing **Reopen Item** button triggers — both paths now flow through rollback
...
**Audit trail:** Every rollback writes a system note to the item's Repair History showing the from/to statuses and which device performed it — e.g. *"Status rolled back from In Progress to Disassembly by Front Counter (front_counter)."* Anyone reading the timeline later can see exactly what happened and when. The **Reopen Item** button on closed items writes the same kind of note now too.

**Important:** Rollback can't be undone — to advance the item back to where it was, the work has to be re-done through the normal flow. Always confirm with whoever's working the item before you roll back their progress.
